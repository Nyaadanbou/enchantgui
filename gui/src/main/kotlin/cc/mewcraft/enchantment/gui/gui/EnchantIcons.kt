    ...
    private inner class ItemProviderCacheLoader : CacheLoader<UiEnchant, Array<ItemProvider>>() {
        override fun load(enchant: UiEnchant): Array<ItemProvider> {
            ...
            for (level in (min..max)) {
                ...
                lore.arguments("menu.enchantment.icon.display_name", enchant.displayName()[level]!!)
                ...
                lore.arguments("menu.enchantment.icon.target", enchant.enchantmentTargets().map { targetTranslator.translate(it) }.reduce { t1, t2 -> "$t1, $t2" })
